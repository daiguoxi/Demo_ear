...
from models.densenet import densenet169 as Net
### Add by Herui --- Imbalance sampler to improve acc
...
# 数据集路径
parser.add_argument('--img_root_train', type=str, default= "E:\\Python项目集\\Pytorch(new)\\深度模型总程序\\nose Fig\\nose Fig\\鼻-鼻窦炎（rhinosinusitis）", help='whether to img root')
# 模型及数据存储路径
parser.add_argument('--checkpoint_dir', type=str, default='E:\\Python项目集\\Pytorch(new)\\深度模型总程序\\ear程序备份\\nose Fig\\nose Fig\\densenet169\\', help='directory where model checkpoints are saved')
# 网络选择
parser.add_argument('--net', dest='net',type=str, default='densenet169',help='which net is chosen for training ')
# 批次
...
if __name__ == '__main__':
    ...
    ### Add by Herui --- Imbalance sampler to improve acc
    #dataloader['train'] = torch.utils.data.DataLoader(data_set['train'], batch_size=opt.batch_size,
                                                      #num_workers=opt.n_cpu, collate_fn=collate_fn,sampler=ImbalancedDatasetSampler(data_set['train']))
    # 载入数据增强后的验证集
    dataloader['val'] = torch.utils.data.DataLoader(data_set['val'], batch_size=opt.batch_size,
                                                    shuffle=True, num_workers=opt.n_cpu, collate_fn=collate_fn)
    #dataloader['val'] = torch.utils.data.DataLoader(data_set['val'], batch_size=opt.batch_size,
                                                     #num_workers = opt.n_cpu, collate_fn = collate_fn,sampler=ImbalancedDatasetSampler(data_set['val']))
    # 获取最大迭代步数
